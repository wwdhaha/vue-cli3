...
function flowersBloom() {
  ...
  tl.staggerTo(
    [flowers, buds, blooms],
    2,
    { scale: 1, ease: Back.easeOut.config(2) },
    0.5,
    0
  )
...
function currentBranchSwaying() {
  ...
  tl.staggerTo(
    currentBranch,
    2 + Math.random(),
    { rotation: currentBranchRotation, ease: Sine.easeInOut },
    Math.random() / 1.2
  );
...
function smallBranchesSway() {
  ...
  tl.staggerTo(
    smallBranches,
    2 + Math.random(),
    { rotation: 5, ease: Sine.easeInOut },
    Math.random() / 1.2,
    "smallBranchSway"
  )
    .to(
      ...
      1 + Math.random(),
      { rotation: -5, transformOrigin: "100% 50%" },
      "smallBranchSway"
    )
    .to(
    ...
    .to(
      ...
      2 + Math.random(),
      { rotation: -5, transformOrigin: "100% 100%" },
      "smallBranchSway"
    );
